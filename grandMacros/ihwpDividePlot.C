...
void ihwpDividePlot(Int_t snlStart, Int_t snlEnd){
  ...
  for(Int_t i = 0; i < snl->GetEntries(); i++){
    snl->GetEntry(i);
    if(sign == 0 || snailNum <= snlStart || snailNum >= snlEnd) continue;
    Int_t ind = getGraphInd(snailNum, hWien, vWien, solWien, ihwp, true);
    Float_t polVar = 100.*pol0.mean*sign; Float_t polErr = 100.*pol0.meanErr*sign;
  ...
  for(Int_t i = 0; i < 4; i++){
    graphs[i]->Fit(Form("fit%i", i), "Q", "", xmin, xmax);
