...
void writeRootfile(vector<vector<vector<Int_t>>> cutInfo, Float_t facStart, Float_t facStep){
  ...
  for(Int_t conf = 0; conf < cutInfo.size(); conf++){
    ...
    TTree *cut = new TTree(Form("cut_%.2f", facStart + facStep*conf), "a tree for cut cycles and their info");
...
void printFactorCuts(Float_t factor){
  ...
  TTree *cut = (TTree *)infile->Get(Form("cut_%.2f", factor));
...
void cutDiffs(Float_t facLo, Float_t facHi){
  ...
  TFile *infile = new TFile("./cutCycles.root", "READ");
  TTree *cutLo = (TTree *)infile->Get(Form("cut_%.2f", facLo));
  TTree *cutHi = (TTree *)infile->Get(Form("cut_%.2f", facHi));
  ...
  while(lo < cutLo->GetEntries() && hi < cutHi->GetEntries()){
    ...
    if(runNumLo == runNumHi && cycNumLo == cycNumHi){
      ...
      printf("Cycle %04i.%02i cut in %.2f but not in %.2f\n", runNumLo, cycNumLo, facLo, facHi);
      lo++;
    }
  }
}
...
void plotRMSDistro(Int_t prexOrCrex, Float_t specFactor, Float_t specFactor2){
  ...
  Float_t facStart = 1.00;
  Float_t facEnd = 2.00;
  Float_t facStep = 0.02;
  ...
  for(Float_t factor = facStart; factor <= facEnd; factor += facStep){
    ...
    for(Int_t i = 0; i < nGroups; i++){
      ...
      TH1F *hOff = new TH1F(hNameOff.Data(), "Laser Off RMS's", 200, 0.0, 1.0);
      TH1F *hOff1 = new TH1F(hNameOff1.Data(), "Laser Off RMS's", 200, 0.0, 1.0);
      TH1F *hOff2 = new TH1F(hNameOff2.Data(), "Laser Off RMS's", 200, 0.0, 1.0);
      TH1F *hOn = new TH1F(hNameOn.Data(), "Laser On RMS's", 200, 0.0, 1.0);
  ...
  Float_t selFacs[5] = {1.0, 1.2, 1.4, 1.6, 1.8};
  for(Float_t factor = facStart; factor <= facEnd; factor += facStep){
    ...
    for(Int_t i = 0; i < cyc->GetEntries(); i++){
      ...
      else if(fullFlag == 0 && atof(Form("%.2f", factor)) == atof(Form("%.2f", selFacs[0]))){hRMS_1->Fill(acc0Off1.rms); hRMS_1->Fill(acc0Off2.rms);}
      else if(fullFlag == 0 && atof(Form("%.2f", factor)) == atof(Form("%.2f", selFacs[1]))){hRMS_2->Fill(acc0Off1.rms); hRMS_2->Fill(acc0Off2.rms);}
      else if(fullFlag == 0 && atof(Form("%.2f", factor)) == atof(Form("%.2f", selFacs[2]))){hRMS_3->Fill(acc0Off1.rms); hRMS_3->Fill(acc0Off2.rms);}
      else if(fullFlag == 0 && atof(Form("%.2f", factor)) == atof(Form("%.2f", selFacs[3]))){hRMS_4->Fill(acc0Off1.rms); hRMS_4->Fill(acc0Off2.rms);}
      else if(fullFlag == 0 && atof(Form("%.2f", factor)) == atof(Form("%.2f", selFacs[4]))){hRMS_5->Fill(acc0Off1.rms); hRMS_5->Fill(acc0Off2.rms);}
      else if(fullFlag == 0 && atof(Form("%.2f", factor)) == atof(Form("%.2f", specFactor))){hRMS_spec->Fill(acc0Off1.rms); hRMS_spec->Fill(acc0Off2.rms);}
    }
    cutConf.push_back(runNums);
    cutConf.push_back(cycNums);
    cutConf.push_back(flags);
    allCutConfs.push_back(cutConf);
    printf("RMS Factor %.2f cut %i cycles\n", factor, (Int_t)cutConf[0].size());
  ...
  leg->AddEntry(hRMS_full, "No Cuts"); leg->AddEntry(hRMS_5, Form("Factor=%.2f", selFacs[4]));
  leg->AddEntry(hRMS_4, Form("Factor=%.2f", selFacs[3])); leg->AddEntry(hRMS_3, Form("Factor=%.2f", selFacs[2]));
  leg->AddEntry(hRMS_2, Form("Factor=%.2f", selFacs[1])); leg->AddEntry(hRMS_1, Form("Factor=%.2f", selFacs[0]));
  leg->AddEntry(hRMS_spec, Form("Factor=%.2f", specFactor));
